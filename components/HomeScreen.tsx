    ...
    >
      ...
      <div className="pointer-events-none safe-bottom fixed inset-x-0 bottom-0 z-20 flex justify-center px-5 pt-10">
        <div className="pointer-events-auto w-full max-w-md sm:max-w-lg">
          <HeartLink href="/events/new" className="w-full">
            + Add Event
          </HeartLink>
        </div>
      </div>
  ...
  return (
    ...
    >
      <button
        ...
        className="mb-3 flex w-full items-center justify-between gap-3 rounded-xl py-1 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/35"
      >
...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="mt-6 rounded-3xl border border-surface-border bg-surface px-5 py-8 text-center shadow-[var(--shadow)]">
      ...
      <HeartButton type="button" onClick={onRetry} size="sm" className="mt-5">
        Retry
      </HeartButton>
    </div>
  );
